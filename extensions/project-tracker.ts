...
function sliceLine(theme: Parameters<typeof fg>[0], slice: ProjectState["slices"][number]): string {
  ...
  return `${sliceIcon(slice.status, theme)} ${fg(theme, "accent", `[${slice.priority}] ${slice.id}`)} ${fg(theme, "muted", `${slice.track} · ${slice.strand.name} · ${slice.status} · ${knotText}`)}`;
}
...
function renderProjectTrackerResult(
  ...
  switch (details.action) {
    ...
    case "slice:get": {
      ...
      const knot = activeKnot(slice);
      const summary = `${slice.id} · ${slice.track} · ${slice.status} · ${slice.strand.name}${knot ? ` · ${knot.name} ${criteriaProgressText(knot.success_criteria)}` : ""}`;
    ...
    case "slice:create": {
      ...
      const summary = slice ? `Created ${slice.id} · ${slice.track} · ${slice.strand.name} · ${slice.status}` : line;
      const body = slice ? [
